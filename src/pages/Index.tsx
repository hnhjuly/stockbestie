...
const Index = () => {
  ...
  const loadTickersFromDB = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('tickers').select('ticker').eq('user_id', deviceId).order('added_at', {
        ascending: false
      });
  ...
  const removeTicker = async (ticker: string) => {
    try {
      ...
      const {
        error
      } = await supabase.from('tickers').delete().eq('ticker', ticker).eq('user_id', deviceId);
      if (error) throw error;
    ...
    const initializeApp = async () => {
      ...
      if (tickerList.length === 0) {
        ...
        try {
          const {
            error
          } = await supabase.from('tickers').insert(defaultTickers.map(ticker => ({
            ticker,
            user_id: deviceId
          })));
          if (error) {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-4 md:py-8">
        ...
        <div className="mb-4 md:mb-6 space-y-4">
          ...
          {tickers.length > 0 && <div className="flex flex-wrap gap-2">
              {tickers.map((ticker, index) => <div key={ticker} className={`flex items-center gap-1 px-3 py-1.5 bg-primary/10 text-primary rounded-full text-sm font-medium touch-manipulation ticker-badge animate-scale-in stagger-${Math.min(index + 1, 10)}`}>
                  {ticker}
                  <button onClick={e => {
              e.stopPropagation();
              removeTicker(ticker);
            }} className="hover:bg-primary/20 rounded-full p-1 transition-colors" aria-label={`Remove ${ticker}`}>
                    <X className="h-3 w-3" />
                  </button>
                </div>)}
            </div>}
        ...
        {isLoading ? <div className="flex items-center justify-center py-20 animate-fade-in">
            <div className="text-center space-y-4">
              <RefreshCw className="h-8 w-8 animate-spin mx-auto text-primary" />
              <p className="text-muted-foreground">Loading companies... You may add more to your Stock Bestie</p>
            </div>
          </div> : stocks.length === 0 ? <div className="text-center py-20 animate-fade-in">
            <div className="text-center space-y-4">
              <RefreshCw className="h-8 w-8 animate-spin mx-auto text-primary" />
              <p className="text-muted-foreground">Loading companies... You may add more to your Stock Bestie</p>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  {lastUpdated && <span className="ml-2">
        ...
        <footer className="mt-12 pt-8 pb-6 border-t">
          <div className="max-w-3xl mx-auto text-center space-y-4">
            <p className="text-xs text-muted-foreground leading-relaxed">
              <strong>Disclaimer:</strong> Stock Bestie is for educational purposes only and does not provide financial or investment advice. Market data may be delayed or inaccurate. Always do your own research.
